...
json BuildSaveListState(object oPC, string sState, string sFolder, string sSaveName, string sArea, string sMTime)
{
    ...
    if (sState == "loaded")
    {
        ...
        string sEntryPrefix = "  ";
        if (sFolder == sSelectedFolder)
        {
            sEntryPrefix = "> ";
        }

        json jEntry = JsonArray();
        jEntry = JsonArrayInsert(jEntry, NuiId(NuiButton(JsonString(sEntryPrefix + sSaveName + "\n" + sArea + "\n" + sMTime)), "btn_save_0"));
...
void main()
{
    ...
    if (GetLocalString(oPC, "R36S_SELECTED_SAVE_FOLDER") == "")
    {
